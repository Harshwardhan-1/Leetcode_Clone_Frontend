  ...
  const handleRun = async (id, title) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3000/api/run/runUserCode",
        send,
    ...
    <div className="pp-main-container">
      ...
      <div className="pp-right">
        ...
        <div className="pp-buttons">
          <button
            onClick={() => handleRun(harsh?._id, harsh?.title)}
            className="pp-run-btn"
          >
            {loading ? "Running..." : "Run Code"}
          </button>
          <button className="pp-submit-btn">
            Submit
          </button>
        </div>
        {result && (
          <div className="pp-output-box">
            ...
            <h3>
              ...
              <span
                style={{
                  color:
                    result.message === "all test case passed"
                      ? "green"
                      : "red",
                  marginLeft: "10px"
                }}
            ...
            {result.data.map((tc, index) => (
              <div
                key={index}
                className={`pp-testcase ${tc.status === "Passed"
                  ? "pass"
                  : "fail"
                }`}
              >
                ...
                <p>
                  ...
                  <span
                    style={{
                      color: tc.status === "Passed" ? "green" : "red"
                    }}
                  >
                    {tc.status}
                  </span>
